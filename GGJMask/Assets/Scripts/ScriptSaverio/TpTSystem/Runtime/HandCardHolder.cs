	...
	public class HandCardHolder : MonoBehaviour
	{
		...
		void Swap(int index)
		{
			...
			cards[index].transform
				.DOLocalMove(cards[index].selected ? new Vector3(0, cards[index].selectionOffset, 0) : Vector3.zero,
					0.12f).SetEase(Ease.OutBack);

			selectCard.transform.SetParent(crossedParent);
			selectCard.transform
				.DOLocalMove(selectCard.selected ? new Vector3(0, selectCard.selectionOffset, 0) : Vector3.zero, 0.12f)
